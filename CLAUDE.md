# Forge V4 — El Cerebro de la Fabrica
...
### 2026-09-24: Escapes unicode en heredocs de Bash
- **Error**: ` ` escrito vía heredoc terminó como NBSP literal dentro de un regex (invisible, frágil) y `Intl` es-CO da `64,3%` sin espacio
- **Fix**: Archivos con escapes unicode se escriben con Write/Edit, no con heredoc. Unir signo y cifra SIEMPRE con espacio fijo (`\u00a0`, `ESPACIO_FIJO` en `format.ts`): `$ 12.000` y `64,3 %` con espacio normal se parten en dos líneas en tablas y tarjetas

### 2026-09-24: React 19 lint — setState dentro de useEffect
- **Error**: `react-hooks/set-state-in-effect` falla al hidratar Zustand persist con `useEffect + setState`
- **Fix**: Usar `useSyncExternalStore(store.persist.onFinishHydration, store.persist.hasHydrated, () => false)`

### 2026-09-24: Next.js 16 — middleware ahora es `src/proxy.ts`
- **Fix**: Exportar `proxy()` desde `src/proxy.ts`; `params`/`searchParams`/`cookies()` son async
